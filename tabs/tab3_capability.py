...
def render(df_raw, sg_col, val_col, lsl, usl):
    st.header("📈 3단계: 잠재 및 실효 공정능력지수(Process Capability) 평가")
    st.write("공정의 장/단기 변동을 정량화하고 품질 만족 한계 상태를 진단합니다.")
    
    # 탭 내부 개별 조절 파라미터 배치
    sigma_method = st.radio(
        "단기 분산(σ_within) 산출 메커니즘 조절 방식 선택", 
        ["Pooled Standard Deviation (합동표준편차 공식)", "Subgroup Range Average (R-bar 계산 방식)"],
        horizontal=True
    )
    ...
    if is_normal:
        ...
        if (df_raw[val_col] <= 0).any():
            st.error("❌ Box-Cox 변환 오류: 데이터에 0 또는 음수가 있어 변환을 스킵하고 원본으로 연산합니다.")
            final_df, final_lsl, final_usl = df_raw.copy(), lsl, usl
        else:
            ...
            st.info(f"💡 정규화 완료: Box-Cox 최적 Lambda (λ) = {lambda_val:.4f}를 적용한 스케일 하에서 수식을 연산합니다.")
    ...
    # 연속 확률 밀도 함수선(PDF) 오버레이 종합 히스토그램 구현
